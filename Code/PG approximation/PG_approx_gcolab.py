...
def contour_fun(z, x, h):
    return torch.abs(torch.log((x[3]*z)**x[2]/h) \
                     - x[3]*z - (z/x[4])**2)
...
def get_ar(x, mu_z, sigma_z, h, ze, zp):
    ...
    return torch.sqrt(x[:,5]/(np.pi*L0))
...
def generate_td(x):
    ...
    mu_z, sigma_z = get_sigma_z_and_mu_z(x[:,2], x[:,3], x[:,4])
    ...
    ar = get_ar(x, mu_z, sigma_z, h, ze, zp)

    PG = PG_fun(x[:,0], x[:,1], x[:,2], x[:,3], x[:,4], ar)
    #PG = PG[(x[:,1]/ar)**2 + (x[:,0]/az)**2 + b*x[:,0] - m*torch.log(b*x[:,0]) <= torch.log(1/h)]
    PG[(x[:,1]/ar)**2 + (x[:,0]/az)**2 + b*x[:,0] - m*torch.log(b*x[:,0]) > torch.log(1/h)] = 0
    print("PG is nan", torch.isnan(PG).any())
    #PG = PG_fun(x[:,0] + displacement, x[:,1], x[:,2], x[:,3], x[:,4], ar)
    return (PG-h)/(l-h)
...
def generate_data(lims,
    ...
    x = torch.zeros(td_size+vd_size, 6)#.to(device)
    y = torch.zeros(td_size+vd_size, 1)
    for j in np.arange(0, x.size(dim=1)):
        x[:,j] = lims[j,0] + torch.rand(td_size+vd_size)*(lims[j,1] - lims[j,0]) # low + k*(high-low)
    #x = torch.rand((td_size+vd_size, 6)) # [batch, (z,r;m,b,az,V)]

    # get target output
    y[:,0] = generate_td(x)
    print("x", x.size(dim=0))
    print("y", y.size(dim=0))
    nan_idx = torch.isnan(y[:,0])
    print("nans", nan_idx)

    x_train = np.delete(x[0:td_size], nan_idx[0:td_size], 0)
    y_train = np.delete(y[0:td_size], nan_idx[0:td_size], 0)
    x_valid = np.delete(x[td_size:-1], nan_idx[td_size:-1], 0)
    y_valid = np.delete(y[td_size:-1], nan_idx[td_size:-1], 0)

    print("x_train: ", x_train.size(dim=0))
    print("y_train: ", y_train.size(dim=0))
    print("x_valid: ", x_valid.size(dim=0))
    print("y_valid: ", y_valid.size(dim=0))
...
lims = np.zeros((6,2))
lims[0,0], lims[0,1] = 1e-16, 4 #z_min, z_max; z>0
lims[1,0], lims[1,1] = 0, 5 #r_min, r_max; r>=0
lims[2,0], lims[2,1] = 0, 20 #m_min, m_max;
lims[3,0], lims[3,1] = 0.60, 0.80 #b_min, b_max;
lims[4,0], lims[4,1] = 3.10, 3.20 #az_min, az_max;
#lims[5,0], lims[5,1] = 0, 10 #ar_min, ar_max;
lims[5,0], lims[5,1] = 100, 110 #V_min, V_max; V>=0

test=torch.zeros((1,6))
test[0,0] = 1
test[0,0] = 1
test[0,2] = 2.8 # m
test[0,3] = 0.67 # b
...
conv_channels = []
inner_lin_layers = [4096, 4096, 4096]
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=2500, gamma=0.1)
