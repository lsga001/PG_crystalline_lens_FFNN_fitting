...
def get_za_and_ar(x, z_cut, h, ze, zp):
    ...
    print("z_p: ", zp)
    print("d: ", d)
    L0 = get_L0(x,h,z_cut) - get_L0(x,h,z_cut-d)
...
def generate_td(x):
    ...
    h = PG_fun(z_cut, 0, m,b,az,1)

    #za, ar = get_za_and_ar(x, mu_z, sigma_z, h, ze, zp)
    za, _ = get_za_and_ar(x, z_cut, h, ze, zp)
...
def generate_data(lims,
    ...
    x = torch.zeros(td_size+vd_size, 5)#.to(device)
    ...
    y = y.numpy()
    # Remove outliers
    outliers_0 = (y[:,0] < lims[5,0]) | (y[:,0] > lims[5,1])
    outliers_1 = (y[:,1] < lims[6,0]) | (y[:,1] > lims[6,1])
    outliers_2 = (y[:,2] < lims[7,0]) | (y[:,2] > lims[7,1])
    outliers_3 = (y[:,3] < lims[8,0]) | (y[:,3] > lims[8,1])
...
lims[4,0], lims[4,1] = 0.4, 4 #zp

lims[5,0], lims[5,1] = 1, 15 # Ra_min, Ra_max
lims[6,0], lims[6,1] = 2, 10 # Rp_min, Rp_max
lims[7,0], lims[7,1] = 2, 7 # d_min, d_max
#lims[7,0], lims[7,1] = 1, 7 # Re_min, Re_max
lims[8,0], lims[8,1] = 100, 250 # V_min, V_max

test=torch.zeros((1,5))
test[0,0] = 2.8 # m
test[0,1] = 0.67 # b
test[0,2] = 3.15 # az
#test[0,3] = 106.5 #V
