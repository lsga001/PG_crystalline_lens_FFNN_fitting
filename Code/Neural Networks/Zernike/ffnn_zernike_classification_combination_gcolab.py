...
def train(
    ...
    for epoch in np.arange(epochs):
        ...
        model.eval()
        with torch.no_grad():
            #train_loss = sum(criterion(net(xb.cuda()), yb.cuda()) for xb, yb in train_dl)
            valid_loss = sum(criterion(net(xv.cuda()), yv.cuda()) for xv, yv in valid_dl)
...
def load_model(LOAD_PATH):
    model = torch.jit.load(LOAD_PATH).cuda()
    model.eval()
    return model
...
#NAME_DATASET_TRAIN = 'dataset_train_comb_BIG_comp_ver1.pt'
#NAME_DATASET_VALID = 'dataset_valid_comb_BIG_comp_ver1.pt'
...
net = NeuralNetwork(conv_channels, lin_layers).cuda()
